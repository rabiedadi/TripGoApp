...
const j = schedule.scheduleJob('36 00 * * *', function () {
    let date = new Date();
    ...
    if (fs.existsSync(yesterdayFile)) {
        fs.readFile( yesterdayFile, function(err, data) {
            const msg = {to: 'o.hamidi@esi-sba.dz', from: 'oussolr32@gmail.com', subject: 'Error logs', text: '...',
                ...
                attachments : [{filename: date.getDate()+'-'+(date.getMonth()+1)+'gatewayReport.txt',
...
module.exports = {
    ...
    handleOthers : function(err, req, res, next) {
        if(err.status === 404)
            res.status(404).json({message: '404 Not found',err:err.errors, path : "gatewayAPI"});
        else{
            let date = new Date();
